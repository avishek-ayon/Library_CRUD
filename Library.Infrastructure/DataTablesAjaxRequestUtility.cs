...
namespace Library.Infrastructure
{
    public class DataTablesAjaxRequestUtility
    {
        ...
        public string SearchText
        {
            get
            {
                return RequestData.Where(x=>x.Key
                =="search[value]").FirstOrDefault().Value;
            }
        }
        ...
        private IEnumerable<KeyValuePair<string, StringValues>> RequestData
        {
            get
            {
                ...
                {
                    throw new InvalidOperationException("Http method is not supported, use get or post");
                }
            }
        }
        ...
        public string GetSortText(string[] columnNames)
        {
            ...
            for(var i=0;i<columnNames.Length;i++)
            {
                if(RequestData.Any(x=>x.Key ==$"order[{ i}][column]"))
                {
                    ...
                    var columnValue = RequestData.Where(x => x.Key == $"order[{i}][column]")
                        .FirstOrDefault();
                    var directionValue = RequestData.Where(x => x.Key == $"order[{i}][dir]")
                        .FirstOrDefault();
